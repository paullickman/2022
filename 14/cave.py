...
class Cave:
    def __init__(self, file, floor=False):
        self.floor = floor
        self.paths = []
        ...
        if floor:
            self.width *= 2
            self.paths.append([Point(0,self.depth+2), Point(self.width-1, self.depth+2)])

        self.cave = [['.' for _ in range(self.width+2)] for _ in range(self.depth+3)]
    ...
    def drip(self):
        ...
        while moved:
            ...
            if not(self.floor) and sand.y >= self.depth:
                return False
...
c1 = Cave('test.txt', True)
assert c1.count2() == 93

c1 = Cave('input.txt', True)
print(c1.count2())
